...
const monthlyPercentages = [
  7.8, 7.5, 8.5, 8.7, 8.9, 8.9, 9.0, 9.1, 8.9, 8.7, 7.8, 7.2,
];
...
const generateData = (
  ...
  const monthlyValues = percentages.map((percentage) =>
    Math.round((totalValue * percentage) / 100)
  );
  return {
    labels: [
      "Jan",
...
const SolarPlanning: React.FC = () => {
  ...
  const moduleData: SolarModule[] = [
    ...
    {
      model: "Vertex Monokristalin 550WP",
      rating: 550,
      width: 1.096,
      height: 2.384,
      price: 1670000,
  ...
  const [lastRectangleBounds, setLastRectangleBounds] =
  ...
  const [selectedModule, setSelectedModule] =
    useState<SolarModule>(defaultModule);
  const [customModule, setCustomModule] = useState<{ [key: string]: string }>({
    model: "",
    rating: "",
    width: "",
    height: "",
  ...
  const addSolarPanel = () => {
    if (mapRef.current) {
      ...
      if (bounds) {
        ...
        newNorthEast = new google.maps.LatLng(
          bounds.getNorthEast().lat(),
          bounds.getNorthEast().lng() + 0.000019
        );
        newSouthWest = new google.maps.LatLng(
          bounds.getSouthWest().lat(),
          bounds.getSouthWest().lng() + 0.000019
        );
      ...
      newRectangle.addListener("dblclick", () => {
        ...
        solarPanel.current = solarPanel.current.filter(
          (rect) => rect !== newRectangle
        ); // Remove from the array
      });
      // Add an event listener on the rectangle.
      google.maps.event.addListener(mapRef.current, "idle", () =>
        showNewRect(newRectangle, selectedModule.rating)
      );
      // Define an info window on the map.
      infoWindow = new google.maps.InfoWindow();

      // Update lastRectangleBounds with the new rectangle's bounds
      setLastRectangleBounds(
  ...
  function boundChangeEvent(rectangle: google.maps.Rectangle) {
    ...
    if (bounds) {
      ...
      const newNorthEast = new google.maps.LatLng(
        bounds.getNorthEast().lat() - latOffset,
        bounds.getNorthEast().lng() - lngOffset
      );
      const newSouthWest = new google.maps.LatLng(
        bounds.getSouthWest().lat() + latOffset,
        bounds.getSouthWest().lng() + lngOffset
      );
      setLastRectangleBounds(
        new google.maps.LatLngBounds(newSouthWest, newNorthEast)
      );
  ...
  async function showNewRect(sp: google.maps.Rectangle, rating: number) {
    ...
    await fetch(url)
      ...
      .then((data) => {
        ...
        const pvout_multiplied = pvout_all.map(
          (value: number) => value * installationSize
        );
        ...
        if (energyType === "Monthly") {
          annualConsumption = energyConsumption.monthly.reduce(
        ...
        chartData1 = generateData(
          "Electricity Usage (kWh)",
          monthlyPercentages,
          energyConsumption.annual
        );
        if (energyType != "Annual") {
          chartData1 = {
        ...
        excessUnderData = chartData2.datasets[0].data.map(
          (value, index) => value - chartData1.datasets[0].data[index]
        );
        chartData3 = {
          ...chartData1,
          datasets: [
          ...
          datasets: [
            {
              ...
              backgroundColor: excessUnderData.map((val) =>
                val > 0 ? "blue" : "red"
              ),
            },
          ],
  ...
  var excessUnderData = chartData2.datasets[0].data.map(
    (value, index) => value - chartData1.datasets[0].data[index]
  );
  var chartData3 = {
    ...chartData1,
        ...
        <div>
          <div className="mt-6 gap-10 flex">
                  ...
                  <TableBody>
                    {moduleData.map((module, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          <Button onClick={() => handleSelect(module)}>
                            Select
                          </Button>
                        </TableCell>
                      </TableRow>
            ...
            <div>
              ...
              <p>
                <strong>Model:</strong> {selectedModule.model}
              </p>
              <p>
                <strong>Rating:</strong> {selectedModule.rating} W
              </p>
              <p>
                <strong>Width:</strong> {selectedModule.width} m
              </p>
              <p>
                <strong>Height:</strong> {selectedModule.height} m
              </p>
            </div>
          </div>
        </div>
              ...
              energyConsumption.monthly.map((value: string, index: number) => (
                <div className="mt-4" key={index}>
                  ...
                  <input
                    ...
                    value={value}
                    onChange={(e) =>
                      handleEnergyInputChange(index, e.target.value)
                    }
                    className="border p-2 ml-2"
                  />
                </div>
      ...
      >
        <div className="p-4 mt-4 rounded-lg w-full max-w-4xl">
          ...
          <div>
            ...
            <p>
              Average Potential (annual) : {resultData?.averagePotential}{" "}
              kWh/kWp
            </p>
            <p>Cost : Rp{resultData?.installationCost}</p>
            <p className="mb-4">
              Optimum Tilt : {resultData?.optimumTilt} degree
            </p>
            <div className="flex justify-around">
              ...
              <div className="text-center">
                ...
                />
                <p className="mt-2">
                  <b>Equal {resultData?.treesNeeded}</b> <br></br>Tree Carbon
                  Absorption
                </p>
              </div>
              ...
              <div className="text-center">
                <img
                  src="/images/logo/green_energy.png"
                  alt="Green Energy"
                  className="h-20 w-20 mx-auto"
                />
                <p className="mt-2">
                  <b>{resultData?.greenEnergy} kWh</b> <br></br>Green Energy
                </p>
              </div>
              ...
              <div className="text-center">
                <img
                  src="/images/logo/co2.png"
                  alt="CO2"
                  className="h-20 w-20 mx-auto"
                />
                <p className="mt-2">
                  <b>{resultData?.co2Saved} Kg</b> <br></br>CO2 Avoided
                </p>
              </div>
            </div>
          </div>
